			...
			function() {
				$("#delete-site").click(
						function() {
							...
							$(".deleteButton").attr(
									"href",
									"LeaveController?action=delete&delBeginDate="
											+ row);
						});
			});
</script>
...
<body>
	...
	<div class="modal fade" id="SuppressionModal" tabindex="-1"
		role="dialog" aria-labelledby="SuppressionModalLabel"
		aria-hidden="true">
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="SuppressionModalLabel">Validation
						suppression</h5>
	...
	<div class="modal fade" id="VisualisationConge" tabindex="-1"
		...
		<div class="modal-dialog" role="document">
			<div class="modal-content">
				<div class="modal-header">
					<h5 class="modal-title" id="VisualisationCongeLabel">Visualisation
						conge</h5>
					<button type="button" class="close" data-dismiss="modal"
						aria-label="Close">
						<span aria-hidden="true">&times;</span>
				...
				<div class="modal-body">
					<div class="row">
						<div class="col">
							<p>Date de debut :
								${listLeavesApproved.get(test).getBeginDate()}</p>
							<p>Date de fin : ${listLeavesApproved.get(test).getEndDate()}</p>
							<p>Duree du conge :
								${listLeavesApproved.get(test).getEndDate()}</p>
						</div>
					</div>
				</div>
			</div>
		...
		<div class="row">
			...
			<div class="col-2 align-self-center">
				<button class="btn btn-secondary"
					onclick="location.href='LeaveController?page=DemandeConge'">Nouveau
					congé</button>
			</div>
		</div>
		<table class="table table-bordered">
				...
				<c:forEach items="${listLeaves}" var="item" varStatus="loop">
					<c:if test="${item.getValidDate() == null}">
						<tr>
							...
							<td class="text-center"><input class="btn btn-outline-dark"
								type="button" id="modifie-site" value="Modifier" /> <input
								class="btn btn-outline-dark" data-id="${item.getBeginDate()}"
								data-toggle="modal" data-target="#SuppressionModal"
								type="button" id="delete-site" value="Supprimer" /> <input
								class="btn btn-outline-dark btn-sm" data-toggle="modal"
								data-target="#VisualisationConge" type="button" id="view_info"
								value="..." /></td>
						</tr>
					</c:if>
				</c:forEach>
					...
					<c:if test="${item.getValidDate() != null}">
						<tr>
							...
							<td class="text-center"><input
								class="btn btn-outline-dark btn-sm" type="button" id="view_info"
								onclick="${System.out.println(item.getType());selectedLeave=item}"
								data-toggle="modal" data-target="#VisualisationConge"
								value="..." /></td>
						</tr>
					</c:if>
				</c:forEach>
